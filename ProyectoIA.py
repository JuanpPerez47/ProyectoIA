...
def graficar_importancia_caracteristicas(modelo):
    nombres_caracteristicas = ['ANTIG','COMPS', 'PROMS','CATEG', 'COMINTS', 'COMPPRESS', 'RATES','VISIT', 'DIASSINQS', 'TASARETS', 'NUMQS', 'RETRES']
    importancias = modelo.feature_importances_
    y_pos = np.arange(len(nombres_caracteristicas))

    plt.figure(figsize=(10, 6))
    plt.barh(y_pos, importancias, color='skyblue', edgecolor='black')
    plt.xlabel('Importancia', fontsize=12)
    plt.ylabel('Características', fontsize=12)
    plt.title('Importancia de las Características', fontsize=14)
    plt.yticks(y_pos, nombres_caracteristicas, fontsize=10)
...
def main():
    ...
    if st.button('RUN'):
        ...
        graficar_importancia_caracteristicas(modelo)
